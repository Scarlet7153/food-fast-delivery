...
droneSchema.virtual('restaurant', {
  ref: 'Restaurant',
...
// Virtual for current mission info
droneSchema.virtual('mission', {
  ref: 'DeliveryMission',
  localField: 'currentMission',
  foreignField: '_id',
  justOne: true
});
...
droneSchema.methods.isWithinGeofence = function(longitude, latitude) {
  if (!this.geofence) return true;
  
  if (this.geofence.type === 'circle') {
    const distance = this.calculateDistance(
      this.geofence.center[1], this.geofence.center[0], // lat, lng
      latitude, longitude
    );
    return distance <= this.geofence.radiusKm;
  }
  
  if (this.geofence.type === 'polygon') {
    return this.isPointInPolygon(longitude, latitude, this.geofence.coordinates);
  }
...
// Instance method to check if point is in polygon
droneSchema.methods.isPointInPolygon = function(x, y, polygon) {
  let inside = false;
