...
// Virtual for mission info
orderSchema.virtual('mission', {
  ref: 'DeliveryMission',
...
orderSchema.methods.calculateEstimatedDeliveryTime = function() {
  const restaurant = this.populate('restaurant');
  if (!restaurant || !restaurant.deliverySettings) {
    return null;
  }
  
  const prepTime = restaurant.deliverySettings.estimatedPrepTime || 15; // minutes
