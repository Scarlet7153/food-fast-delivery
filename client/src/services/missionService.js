...
const missionService = {
  ...
  async getRestaurantMissions(params = {}) {
    const response = await api.get('/missions/restaurant', { params })
  ...
  async updateMissionStatus(missionId, status, note = '') {
    const response = await api.patch(`/missions/${missionId}/status`, { status, note })
    return response.data
  },
  
  // Assign drone to mission
  async assignDroneToMission(missionId, droneId) {
    const response = await api.post(`/missions/${missionId}/assign-drone`, { droneId })
    return response.data
  },
  
  // Get mission tracking info
  async getMissionTracking(missionId) {
    const response = await api.get(`/missions/${missionId}/tracking`)
    return response.data
  },
  
  // Get mission location updates
  async getMissionLocationUpdates(missionId) {
    const response = await api.get(`/missions/${missionId}/locations`)
    return response.data
  },
  
  // Complete mission
  async completeMission(missionId, deliveryNote = '') {
    const response = await api.post(`/missions/${missionId}/complete`, { deliveryNote })
    return response.data
  },
