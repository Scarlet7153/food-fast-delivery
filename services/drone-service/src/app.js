...
class DroneService {
  ...
  setupMiddleware() {
    ...
    this.app.use(metricsMiddleware('drone-service'));
  ...
  setupErrorHandling() {
    ...
    this.app.use((error, req, res, next) => {
      logger.error('Drone Service Error:', error);
      res.status(500).json({ 
  ...
  async start() {
    try {
      ...
      this.server.listen(config.PORT, () => {
        logger.info(`Drone Service running on port ${config.PORT}`);
      ...
      logger.error('Failed to start Drone Service:', error);
      process.exit(1);
    }
  }
  ...
  async shutdown() {
    ...
    try {
      ...
      logger.info('Drone Service shut down successfully');
      process.exit(0);
    } catch (error) {
...
droneService.start().catch((error) => {
  logger.error('Failed to start Drone Service:', error);
  process.exit(1);
});
